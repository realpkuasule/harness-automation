...
A plan compiles only what its arguments name. On a project that already carries `.harness/policy.yaml`, an omitted `--delivery-profile`, `--domain-profile`, or `--quality-profile` inherits the applied value instead of dropping it, so the suites those profiles configure — including the pinned evaluation contract — survive a re-plan. An explicitly supplied profile list always wins.
