...
import { applyCredentialHostBinding, loadCredentialHostBinding, planCredentialHostBinding, type CredentialBindingPlan, type CredentialHostBinding } from "./host_binding.js";
...
export function runCredentialCommand(projectRoot: string, args: ParsedArguments): unknown {
  ...
  const requireEndpoint = (binding: CredentialHostBinding | Omit<CredentialHostBinding, "bindingHash">) => {
  ...
  if (args.positionals[0] === "plan") {
    const input = read("input") as Omit<CredentialHostBinding, "bindingHash">;
    ...
    const approval = createSemanticApprovalPacket({
      ...
      binding: {
        ...
        observedHash: hashObject({ beforeHash: plan.beforeHash, beforeLkgHash: plan.beforeLkgHash, worktreeBindingHash: plan.worktreeBindingHash }),
      },
      actions: [{ id: "credential-registration", kind: "permission-change", protected: true, reversible: true,
        summary: `Register ${input.credentials.length} credential reference(s) for ${repository}; no secret is read.`,
    ...
    if (!existsSync(planPath)) { durableWriteOnce(planPath, prettyJson(plan)); chmodSync(planPath, 0o600); }
