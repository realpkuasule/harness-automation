...
describe("strict coordination records", () => {
  it("keeps documented record fields aligned with the strict runtime boundary", () => {
    const schema = JSON.parse(readFileSync(new URL("../../../docs/api/coordination-v1.schema.json", import.meta.url), "utf8"));
    expect(Object.keys(schema.properties).sort()).toEqual(Object.keys(coordinationRecordSchema.shape).sort());
  });
