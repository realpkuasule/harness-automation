...
import { objectDirectory, objectEnv } from "./objects.js";
...
function requireObjectDirectory(directory: string): void {
  ...
  const result = spawnSync("git", ["config", "--file", join(directory, "config"), "--no-includes", "--null", "--list"], { env: objectEnv(), encoding: "utf8", timeout: 10_000, maxBuffer: 8192 });
