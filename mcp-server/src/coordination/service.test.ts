...
describe("GitCoordinationStore", { timeout: 20_000 }, () => {
  ...
  it("does not let a delayed renewal extend a lease without pre-expiry server evidence", () => {
    const first = lease();
    const pending = reserveRenewal(first, expectedRecord(first), "2031-01-01T00:00:00.000Z");
    expect(() => confirmRenewal(pending, expectedRecord(pending), "2030-01-01T00:00:00.000Z")).toThrow("COORDINATION_RENEWAL_TIME_UNPROVEN");
    const renewed = confirmRenewal(pending, expectedRecord(pending), "2029-12-31T23:59:59.000Z");
