...
const cases = [
  ...
  ["local-assets", "src/worktree/qualification.ts", ["assets.entries.length", "readdirSync(resource.path).some((name) => name !== \".git\")", "if (progress.phase === \"ready\" && progress.evidenceHash !== evidenceHash) throw new Error(\"QUALIFICATION_RESOURCE_METADATA_DRIFT\");"], ["false", "false", "if (false) throw new Error(\"QUALIFICATION_RESOURCE_METADATA_DRIFT\");"], "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw an error"],
  ...
  ["local-directory-identity", "src/worktree/qualification.ts", "return { device: stat.dev, inode: stat.ino, birthtimeMs: stat.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "return { device: parent.dev, inode: parent.ino, birthtimeMs: parent.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw an error"],
