...
const cases = [
  ...
  ["local-assets", "src/worktree/qualification.ts", "if (assets.entries.length || inspectGit(resource.path, [\"ls-files\", \"--stage\", \"-z\"]) ||\n      readdirSync(resource.path).some((name) => name !== \".git\")) throw new Error(\"QUALIFICATION_RESOURCE_ASSETS_RETAINED\");", "if (false) throw new Error(\"QUALIFICATION_RESOURCE_ASSETS_RETAINED\");", "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw error including 'QUALIFICATION_RESOURCE_ASSETS_RETAINED' but got 'QUALIFICATION_RESOURCE_METADATA_DRIFT'", "src/coordination/github.test.ts:327:"],
  ...
  ["local-directory-identity", "src/worktree/qualification.ts", "return { device: stat.dev, inode: stat.ino, birthtimeMs: stat.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "return { device: parent.dev, inode: parent.ino, birthtimeMs: parent.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw an error", "src/coordination/github.test.ts:333:"],
