...
const cases = [
  ...
  ["local-assets", "src/worktree/qualification.ts", "if (assets.entries.length || inspectGit(resource.path, [\"ls-files\", \"--stage\", \"-z\"]) ||\n      readdirSync(resource.path).some((name) => name !== \".git\")) throw new Error(\"QUALIFICATION_RESOURCE_ASSETS_RETAINED\");", "if (false) throw new Error(\"QUALIFICATION_RESOURCE_ASSETS_RETAINED\");", "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw error including 'QUALIFICATION_RESOURCE_ASSETS_RETAINED' but got 'QUALIFICATION_RESOURCE_METADATA_DRIFT'", "src/coordination/github.test.ts:301:"],
  ...
  ["local-directory-identity", "src/worktree/qualification.ts", "return { device: stat.dev, inode: stat.ino, birthtimeMs: stat.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "return { device: parent.dev, inode: parent.ino, birthtimeMs: parent.birthtimeMs, parentDevice: parent.dev, parentInode: parent.ino, parentBirthtimeMs: parent.birthtimeMs };", "src/coordination/github.test.ts", "creates approved synthetic worktrees without touching primary content or erasing failure (complete)", "AssertionError: expected [Function] to throw an error", "src/coordination/github.test.ts:307:"],
  ...
  ["local-close-branch-drift", "src/worktree/qualification.ts", "  if (currentBranch && currentBranch !== resource.sourceSha) throw new Error(\"BRANCH_REF_DRIFT\");", "  if (false) throw new Error(\"BRANCH_REF_DRIFT\");", "src/coordination/github.test.ts", "retains when the local branch was advanced past the recorded source SHA", "AssertionError: expected [Function] to throw error including 'BRANCH_REF_DRIFT'", "src/coordination/github.test.ts:826:"],
  ["local-runtime-context-leak", "src/worktree/qualification_runtime.ts", "  if (scope.localResources.configHash !== scope.binding.configHash) {\n    throw new Error(\"HUMAN_AUTHORIZATION_BINDING_MISMATCH\");", "  if (false) {\n    throw new Error(\"HUMAN_AUTHORIZATION_BINDING_MISMATCH\");", "src/worktree/qualification.test.ts", "rejects a binding drift between localResources.configHash and binding.configHash (HUMAN_AUTHORIZATION_BINDING_MISMATCH)", "AssertionError: expected [Function] to throw an error", "src/worktree/qualification.test.ts:182:"],
  ["local-acquire-contention", "src/coordination/push_result.ts", "  if (outcome === \"rejected\") throw new Error(\"COORDINATION_CAS_CONFLICT\");", "  if (false) throw new Error(\"COORDINATION_CAS_CONFLICT\");", "src/coordination/lifecycle-acquire.test.ts", "two A workers competing for the same control ref produce one winner and one COORDINATION_CAS_CONFLICT loser", "AssertionError: expected [Function] to throw error including 'COORDINATION_CAS_CONFLICT'", "src/coordination/lifecycle-acquire.test.ts:56:"],
  ["local-disposable-drift", "src/worktree/service.ts", "          operation.disposedIgnoredPathCount !== undefined && operation.disposedIgnoredPathCount !== disposed.length ||\n          operation.disposedIgnoredPathsHash !== undefined && operation.disposedIgnoredPathsHash !== hashObject(disposed)) {", "          false ||\n          false) {", "src/worktree/service.test.ts", "rejects a close whose disposable ignored set changed after the owner approved the plan", "AssertionError: expected [Function] to throw an error", "src/worktree/service.test.ts:2097:"],
];
