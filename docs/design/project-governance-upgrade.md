...
`legacyEvalSnapshotMigration` is valid only when the before policy has no snapshot, the candidate has a traceable snapshot, and old/current eval sources differ. It includes `{kind:"legacy-eval-snapshot-adoption", historicalContinuity:"unavailable", legacyPolicyDigest, historicalEvalSources, currentApprovedEvalSources, candidateEvaluations, affectedSuites}`. Apply derives it again from the before/after policy and rejects a mismatch before any write. On success, the existing immutable `.harness/changes/<id>/change.json` receipt gains an optional migration section with the owner, `{evaluationsSnapshot:"absent", policyDigest}` before state, and `{policyDigest, evaluationsSha256, approvedEvalSources}` after state; top-level `appliedAt` and `planHash` remain authoritative. This is additive only: old receipts are never rewritten and rollback remains unchanged. Future ordinary updates compare against the persisted candidate snapshot.
